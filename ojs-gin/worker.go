...
func (wm *WorkerManager) Stop() error {
	...
	case <-done:
		return wm.Err()
	case <-timer.C:
		return fmt.Errorf("ojsgin: worker shutdown exceeded %s", timeout)
	}
}
...
func (wm *WorkerManager) Wait() error {
	...
	if !wm.started {
		wm.mu.RUnlock()
		return fmt.Errorf("ojsgin: worker has not been started")
	}
	done := wm.done
	wm.mu.RUnlock()
...
func (wm *WorkerManager) prepareStart(ctx context.Context) (*ojs.Worker, context.Context, chan struct{}, error) {
	if ctx == nil {
		return nil, nil, nil, fmt.Errorf("ojsgin: context must not be nil")
	}

	wm.mu.Lock()
	defer wm.mu.Unlock()
	if wm.worker == nil {
		return nil, nil, nil, fmt.Errorf("ojsgin: no handlers registered; call Register before Start")
	}
	if wm.started {
		return nil, nil, nil, fmt.Errorf("ojsgin: worker has already been started")
	}
