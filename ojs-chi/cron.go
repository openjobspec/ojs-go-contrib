...
func RegisterCrons(ctx context.Context, client *ojs.Client, crons []CronConfig) error {
	if ctx == nil {
		return fmt.Errorf("ojschi: context must not be nil")
	}
	for i, c := range crons {
		if strings.TrimSpace(c.Name) == "" ||
			strings.TrimSpace(c.Schedule) == "" ||
			strings.TrimSpace(c.JobType) == "" {
			return fmt.Errorf("ojschi: cron %d requires Name, Schedule, and JobType", i)
		}
		if client == nil {
			return fmt.Errorf("ojschi: OJS client must not be nil")
		}
