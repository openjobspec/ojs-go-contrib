...
func RegisterCrons(ctx context.Context, client *ojs.Client, crons []CronConfig) error {
	if ctx == nil {
		return fmt.Errorf("ojsfiber: context must not be nil")
	}
	for i, c := range crons {
		if strings.TrimSpace(c.Name) == "" ||
			strings.TrimSpace(c.Schedule) == "" ||
			strings.TrimSpace(c.JobType) == "" {
			return fmt.Errorf("ojsfiber: cron %d requires Name, Schedule, and JobType", i)
		}
		if client == nil {
			return fmt.Errorf("ojsfiber: OJS client must not be nil")
		}
		...
		if _, err := client.RegisterCronJob(ctx, req); err != nil {
			return fmt.Errorf("ojsfiber: registering cron %q: %w", c.Name, err)
		}
	}
	return nil
}
